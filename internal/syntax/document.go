...
var commonFieldRules = []FieldRule{
	{Name: "name", Required: true, Status: FieldCore},
	{Name: "description", Required: true, Status: FieldCore},
	{Name: "license", Status: FieldCore},
	{Name: "compatibility", Status: FieldCore},
	{Name: "metadata", Status: FieldCore},
	{Name: "allowed-tools", Status: FieldCore},
}

var claudeFieldRules = []FieldRule{
	{Name: "arguments", Status: FieldSupported},
	{Name: "disallowed-tools", Status: FieldSupported},
	{Name: "globs", Status: FieldSupported},
...
func ParseDocument(source, path string, host Host) (Document, []warning.Warning) {
	document := Document{
		Host:     host,
		Metadata: []MetadataEntry{}, Arguments: []string{}, AllowedTools: []ToolPattern{},
	...
	for _, entry := range entries {
		line := entry.line + 1
		if entry.name == "" {
			warnings = append(warnings, syntaxWarning(warning.WarnSyntaxInvalidFrontmatter, path, line, "frontmatter entry is invalid"))
			continue
		}
		rule, known := ruleByName[entry.name]
		if !known {
			warnings = append(warnings, syntaxWarning(warning.WarnSyntaxUnknownField, path, line, "frontmatter field is unknown"))
			continue
		}
		if seen[entry.name] {
			warnings = append(warnings, syntaxWarning(warning.WarnSyntaxDuplicateField, path, line, "frontmatter field is duplicated"))
			continue
		}
		seen[entry.name] = true
		if rule.Status == FieldUnsupported {
			warnings = append(warnings, syntaxWarning(warning.WarnSyntaxUnsupportedField, path, line, "frontmatter field is unsupported"))
			continue
		}
		if entry.err != nil {
			...
			warnings = append(warnings, syntaxWarning(warning.WarnSyntaxInvalidField, path, line, "frontmatter field is invalid"))
			continue
		}
		projected := projectDocumentField(&document, entry.field, path, line)
		if len(projected) == 0 {
	...
	if !validDocumentName(document.Name) && valid["name"] {
		warnings = append(warnings, syntaxWarning(warning.WarnSyntaxInvalidField, path, entryLine(entries, "name")+1, "frontmatter name is invalid"))
	}
	if strings.TrimSpace(document.Description) == "" && valid["description"] {
		warnings = append(warnings, syntaxWarning(warning.WarnSyntaxInvalidField, path, entryLine(entries, "description")+1, "frontmatter description is invalid"))
	}
...
func projectDocumentField(document *Document, field YAMLField, path string, line int) []warning.Warning {
	...
	case "metadata":
		if field.Value.Kind != YAMLMapping {
			return invalid("a scalar mapping")
		}
		seen := make(map[string]bool)
		var warnings []warning.Warning
		for _, entry := range field.Value.Mapping {
			...
			if seen[entry.Name] {
				warnings = append(warnings, syntaxWarning(warning.WarnSyntaxDuplicateField, path, entryLine, "metadata field is duplicated"))
				continue
			}
			seen[entry.Name] = true
			value, ok := yamlString(entry.Value)
			if !ok {
				warnings = append(warnings, syntaxWarning(warning.WarnSyntaxInvalidField, path, entryLine, "metadata field is invalid"))
				continue
			}
			document.Metadata = append(document.Metadata, MetadataEntry{Name: entry.Name, Value: value})
		}
		return warnings
	case "arguments", "globs":
		values, ok := yamlScalarItems(field.Value)
		if !ok {
			return invalid("a scalar or scalar sequence")
		}
		if field.Name == "arguments" {
			...
			for _, item := range values {
				...
				if !validArgumentName(item.value) {
					warnings = append(warnings, syntaxWarning(warning.WarnSyntaxInvalidField, path, itemLine, "argument name is invalid"))
					continue
				}
				if seen[item.value] {
					warnings = append(warnings, syntaxWarning(warning.WarnSyntaxDuplicateArgument, path, itemLine, "argument name is duplicated"))
					continue
				}
