...
func TestReleaseOneShotPublicAPISurface(t *testing.T) {
	want := []string{
		...
		"const CodeModelCallLimit ErrorCode = \"model_call_limit\"",
		...
		"func Probe func(ctx context.Context, request ProbeRequest) (Result, error)",
		...
		"type ProbeRequest struct{Model google.golang.org/adk/v2/model.LLM; MaxOutputTokens int32}",
		"type Request struct{Model google.golang.org/adk/v2/model.LLM; Instruction string; Prompt string; Tools []google.golang.org/adk/v2/tool.Tool; MaxOutputTokens int32; MaxReturnedTextBytes int; MaxModelCalls int; MaxToolCallsPerResponse int; ToolExecution ToolExecutionPolicy}",
		"type Result struct{Text string; ToolResults []ToolResult; Metadata Metadata}",
		"type ToolExecutionPolicy uint8",
		"type ToolResult struct{ID string; Name string; Response map[string]any}",
