...
var approvedContextCallables = []callableApproval{
	...
	nestedCallableApproval("oneshot/boundaries.go", "variable descriptor in func protectTools via pointer element -> field processor -> method ProcessRequest", "func(google.golang.org/adk/v2/agent.Context, *google.golang.org/adk/v2/model.LLMRequest) error", "inspected caller request processor passed into the one-shot wrapper"),
